...
final class GitHubProvider
{
    ...
    private function buildBody(EcouteCapture $capture, array $aiResponse, ?string $template, ?string $captureId): array
    {
        ...
        $codeSuggestion = $aiResponse['code_suggestion'] ?? null;
        if (is_string($codeSuggestion) && $codeSuggestion !== '') {
            $body .= "\n\n".$this->codeSuggestionBlock($codeSuggestion);
        }
    ...
    /**
     * Wrap a code suggestion in a collapsible GitHub Markdown details block.
     */
    private function codeSuggestionBlock(string $codeSuggestion): string
    {
        return "<details>\n<summary>🔧 Suggested code change</summary>\n\n{$codeSuggestion}\n\n</details>";
    }

    /**
     * Create a GitHub pull request with the AI-suggested code change.
     * Returns the PR HTML URL, or null when the code_suggestion is not parseable.
     *
     * @param  array<string, mixed>  $aiResponse
     */
    public function createPullRequest(EcouteCapture $capture, array $aiResponse): ?string
    {
        $codeSuggestion = $aiResponse['code_suggestion'] ?? null;
        if (! is_string($codeSuggestion) || $codeSuggestion === '') {
            Log::info('Ecoute: auto-pr skipped — no code_suggestion in AI response', ['capture_id' => $capture->id]);

            return null;
        }

        $parsed = $this->parseCodeSuggestion($codeSuggestion);
        if ($parsed === null) {
            Log::info('Ecoute: auto-pr skipped — unparseable code_suggestion', ['capture_id' => $capture->id, 'suggestion' => substr($codeSuggestion, 0, 200)]);

            return null;
        }

        ['file_path' => $filePath, 'content' => $content] = $parsed;

        $baseBranch = (string) ($this->config['auto_pr']['base_branch'] ?? 'main');
        Log::info('Ecoute: creating auto-pr', ['capture_id' => $capture->id, 'file' => $filePath, 'branch' => $baseBranch]);
        $prTitle = 'Fix: '.($aiResponse['title'] ?? 'Ecoute suggested fix');
        $owner = $this->config['owner'];
        $repo = $this->config['repo'];
        $api = "https://api.github.com/repos/{$owner}/{$repo}";

        $shortId = substr((string) $capture->id, 0, 8);
        $interaction = $capture->interaction ?? [];
        $pageUrl = $interaction['url'] ?? 'Unknown';
        $pageTitle = $interaction['page_title'] ?? 'Unknown';

        $prBody = "## AI-Suggested Fix\n\n";
        $issueUrl = $capture->github_issue_url ?? '';
        if ($issueUrl) {
            $prBody .= "> Fixes issue: {$issueUrl}\n\n";
        }

        $prBody .= '**Description:** '.($aiResponse['description'] ?? 'N/A')."\n\n";
        $prBody .= '**Suggested Fix:** '.($aiResponse['suggested_fix'] ?? 'N/A')."\n\n";
        $prBody .= "**Page:** [{$pageTitle}]({$pageUrl})\n";
        $prBody .= "**Element:** `{$capture->element_selector}`\n\n";
        $prBody .= "---\n\n## Code Change\n\n{$codeSuggestion}\n\n";
        $prBody .= $this->diagnosticsBlock($capture);
        $prBody .= "\n\n> 🤖 Generated by [Ecoute](https://github.com/mikehins/ecoute) — AI-assisted issue capture";

        // 1. Get base branch HEAD SHA
        $baseRef = $this->githubRequest()->get("{$api}/git/refs/heads/{$baseBranch}");
        if (! $baseRef->successful()) {
            Log::error('Ecoute: auto-pr failed — base branch not found', ['capture_id' => $capture->id, 'branch' => $baseBranch, 'status' => $baseRef->status()]);

            return null;
        }
        $baseSha = $baseRef->json('object.sha');

        // 2. Create branch from master's HEAD (this SHA definitely exists)
        $branchName = 'ecoute-fix-'.$shortId.'-'.substr(md5(uniqid('', true)), 0, 6);

        $ref = $this->githubRequest()->post("{$api}/git/refs", [
            'ref' => 'refs/heads/'.$branchName,
            'sha' => $baseSha,
        ]);
        if (! $ref->successful()) {
            Log::error('Ecoute: auto-pr failed — cannot create branch from base', ['capture_id' => $capture->id, 'branch' => $branchName, 'base' => $baseBranch, 'sha' => $baseSha, 'status' => $ref->status(), 'body' => substr((string) $ref->body(), 0, 300)]);

            return null;
        }

        Log::info('Ecoute: auto-pr branch created', ['capture_id' => $capture->id, 'branch' => $branchName]);

        // 3. Create a suggestion file on the branch instead of overwriting the source.
        //    AI code suggestions are surgical, not full-file replacements. The PR
        //    shows the suggestion as a markdown file for a developer to review and apply.
        $suggestionFile = '.ecoute/suggestions/'.$shortId.'.md';
        $suggestionBody = "# {$prTitle}\n\n"
            ."**File:** `{$filePath}`\n\n"
            .'**Description:** '.($aiResponse['description'] ?? 'N/A')."\n\n"
            .'**Fix:** '.($aiResponse['suggested_fix'] ?? 'N/A')."\n\n"
            ."---\n\n## Code Suggestion\n\n{$codeSuggestion}\n\n"
            .$this->diagnosticsBlock($capture)
            ."\n\n> 🤖 Generated by [Ecoute](https://github.com/mikehins/ecoute)";

        $putContent = $this->githubRequest()->put("{$api}/contents/{$suggestionFile}", [
            'message' => $prTitle,
            'content' => base64_encode($suggestionBody),
            'branch' => $branchName,
        ]);

        if (! $putContent->successful()) {
            Log::error('Ecoute: auto-pr failed — cannot update file', ['capture_id' => $capture->id, 'status' => $putContent->status(), 'body' => substr((string) $putContent->body(), 0, 300)]);

            return null;
        }

        // 4. Create PR
        $pr = $this->githubRequest()->post("{$api}/pulls", [
            'title' => $prTitle,
            'head' => $branchName,
            'base' => $baseBranch,
            'body' => $prBody,
        ...
        if (! $pr->successful()) {
            Log::error('Ecoute: auto-pr failed — cannot create PR', ['capture_id' => $capture->id, 'status' => $pr->status(), 'body' => substr((string) $pr->body(), 0, 300)]);
    ...
    /**
     * Parse an AI code_suggestion into file path and new content.
     * Extracts the file path from the first line and the After: block content.
     *
     * @return array{file_path: string, content: string}|null
     */
    private function parseCodeSuggestion(string $codeSuggestion): ?array
    {
        $lines = explode("\n", $codeSuggestion);
        $filePath = trim($lines[0] ?? '');

        if ($filePath === '') {
            return null;
        }

        // Strip bold markdown, label prefixes, and trailing bold from file path line.
        // Handles: "**Fichier:** resources/views/home.blade.php", "path/to/file.php", "** path/to/file.php **"
        $filePath = trim($filePath, '* ');
        $filePath = (string) preg_replace('/^[A-Za-zÀ-ÿ]+\s*:\s*/u', '', $filePath);
        $filePath = trim($filePath, '* ');
        ...
        // Find the After:/Après: section (flexible matching)
        $afterIdx = null;
        foreach ($lines as $i => $line) {
            $clean = trim($line);
            $clean = (string) preg_replace('/^\*{1,2}|\*{1,2}$/', '', $clean);
            $cleanLower = mb_strtolower(trim($clean, ': '));

            if (in_array($cleanLower, ['after', 'après', 'apres'], true)) {
                $afterIdx = $i;
                break;
            }
        }
        ...
        $content = '';
        $fenceCount = 0;
        for ($i = $afterIdx + 1; $i < count($lines); $i++) {
            $line = $lines[$i];
        ...
        return ['file_path' => $filePath, 'content' => $content];
    }
